...
var toValidIntMatches = function toValidIntMatches(x) {
  return x.match(VERSION_RX).slice(1).map(function (m) {
    return +m;
  }).filter(function (m) {
    return !isNaN(m);
  });
...
var resolve = function resolve(range, versions, pre) {
  ...
  var _VERSION_RX$exec2 = _slicedToArray(_VERSION_RX$exec, 7);

  var root = _VERSION_RX$exec2[0];
  var prefix = _VERSION_RX$exec2[1];
  var major = _VERSION_RX$exec2[2];
  var minor = _VERSION_RX$exec2[3];
  var patch = _VERSION_RX$exec2[4];
  var partial = _VERSION_RX$exec2[5];
  var beta = _VERSION_RX$exec2[6];
